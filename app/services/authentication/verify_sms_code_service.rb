...
class Authentication::VerifySmsCodeService < Authentication::SmsCodeBaseService
  ...
  def execute
    ...
    return error("验证码不能为空") if @code.blank?
    return error("手机号格式不正确") unless valid_phone?(@phone_number)
    return error("验证码应为6位数字") unless valid_code_format?(@code)
