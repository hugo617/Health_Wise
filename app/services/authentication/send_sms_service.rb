...
# 发送短信验证码服务
class Authentication::SendSmsService < Authentication::SmsCodeBaseService
  ...
  def execute
    # 参数校验
    return error("手机号不能为空") if @phone_number.blank?
    return error("手机号格式不正确") unless valid_phone?(@phone_number)

    # 检查发送频率限制
    rate_limit_result = check_rate_limit(@phone_number)
    ...
    # 记录发送次数
    record_send_attempt(@phone_number)
